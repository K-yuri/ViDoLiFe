...
<body>
	...
	<div id="wrap-all">
		<section id="board_wrap">
			...
			<% if(loginUser != null) { %>
				<div id="write_button">
					<button id="wbtn" onclick="location.href='<%= request.getContextPath()  %>/writeJoinForm.jo'">글작성</button>
				</div>
			<% } %>

			<div id="move_page">
			<!-- 맨 처음 페이지 -->
			<button onclick="location.href='<%= request.getContextPath() %>/list.jo?currentPage=1'">&lt;&lt;</button>
			<!-- 이전 페이지 -->
			<button id="beforeBtn" onclick="location.href='<%= request.getContextPath() %>/list.jo?currentPage=<%= pi.getCurrentPage() - 1 %>'">&lt;</button>
			<script>
			...
			<% 		} else { %>
					<button id="numBtn" onclick="location.href='<%= request.getContextPath() %>/list.jo?currentPage=<%= p %>'"><%= p %></button>
			<% 		} %>
			<% } %>
			<!-- 다음 페이지 -->
			<button id="afterBtn" onclick="location.href='<%= request.getContextPath() %>/list.jo?currentPage=<%= pi.getCurrentPage() + 1 %>'">&gt;</button>
			<script>
			...
			<!-- 맨 뒤 페이지 -->
			<button onclick="location.href='<%= request.getContextPath() %>/list.jo?currentPage=<%= pi.getMaxPage() %>'">&gt;&gt;</button>
			</div>
	...
	<script>
		...
		document.getElementById("search_btn").onclick = function(){ //검색 버튼을 누르면 
			...
			location.href='<%= request.getContextPath() %>/search.jo?field=' + field + '&input=' + input;													
		}
	</script>
</body>
</html>