...
<body>
	...
	<div id="wrap-all">
		<section id="board_wrap">
			...
			<% if(loginUser != null) { %>
				<div id="write_button">
					<button id="wbtn" onclick="location.href='<%= request.getContextPath()  %>/writeReviewForm.re'">글작성</button>
				</div>
			<% } %>
			...
			<!-- 이전 페이지 -->
			<button id="beforeBtn" onclick="location.href='<%= request.getContextPath() %>/list.re?currentPage=<%= pi.getCurrentPage() - 1 %>'">&lt;</button>
			<script>
			...
			<% } %>
			<!-- 다음 페이지 -->
			<button id="afterBtn" onclick="location.href='<%= request.getContextPath() %>/list.re?currentPage=<%= pi.getCurrentPage() + 1 %>'">&gt;</button>
			<script>
			...
			<!-- 맨 뒤 페이지 -->
			<button onclick="location.href='<%= request.getContextPath() %>/list.re?currentPage=<%= pi.getMaxPage() %>'">&gt;&gt;</button>
			</div>
	...
	<script>
		...
		document.getElementById("search_btn").onclick = function(){
			...
			location.href='<%= request.getContextPath() %>/search.re?field=' + field + '&input=' + input;													
		}
	</script>
</body>
</html>